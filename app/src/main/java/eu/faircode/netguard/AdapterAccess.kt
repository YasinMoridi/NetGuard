...
class AdapterAccess(context: Context, cursor: Cursor) : CursorAdapter(context, cursor, 0) {
    ...
    override fun bindView(view: View, context: Context, cursor: Cursor) {
        ...
        // Set values
        tvTime.text = SimpleDateFormat("dd HH:mm", Locale.ROOT).format(Date(time))
        if (block < 0) {
        ...
        when {
            ...
            allowed > 0 -> tvDest.setTextColor(colorOn)
        ...
        when {
            sent > 1024 * 1024 * 1024L || received > 1024 * 1024 * 1024L -> {
                tvTraffic.text = context.getString(
                    R.string.msg_gb,
                    if (sent > 0) sent / (1024 * 1024 * 1024f) else 0f,
                    if (received > 0) received / (1024 * 1024 * 1024f) else 0f
                )
            }
            sent > 1024 * 1024L || received > 1024 * 1024L -> {
                tvTraffic.text = context.getString(
                    R.string.msg_mb,
