...
class ActivityMain : AppCompatActivity(), SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ivQueue?.setOnLongClickListener {
            ...
            toast.setGravity(
                ...
                (location[1] + ivQueue!!.bottom - toast.view!!.paddingTop).toInt()
            )
            toast.show()
            true
        }
        ...
        swEnabled?.setOnCheckedChangeListener { _, isChecked ->
            ...
            if (isChecked) {
                ...
                if (filter && Util.isPrivateDns(this@ActivityMain))
                    Toast.makeText(this@ActivityMain, R.string.msg_private_dns, Toast.LENGTH_LONG).show()
                ...
                ServiceSinkhole.stop("switch off", this@ActivityMain, false)
            }
        }
        if (enabled)
            checkDoze()
        ...
        ivMetered?.setOnLongClickListener {
            ...
            toast.setGravity(
                ...
                (location[1] + ivMetered!!.bottom - toast.view!!.paddingTop).toInt()
            )
            toast.show()
            true
        }
        ...
        tvNotifications.visibility = View.GONE
        ...
        theme.resolveAttribute(R.attr.colorPrimary, tvAttr, true)
        swipeRefresh = findViewById(R.id.swipeRefresh)
        swipeRefresh?.setColorSchemeColors(Color.WHITE, Color.WHITE, Color.WHITE)
        swipeRefresh?.setProgressBackgroundColorSchemeColor(tvAttr.data)
        ...
        if (!initialized) {
            ...
            dialogFirst = AlertDialog.Builder(this)
                ...
                .setNegativeButton(R.string.app_disagree) { _, _ ->
                    if (running)
                        finish()
                }
        ...
        try {
            iab = IAB({ iab ->
                try {
                    ...
                    if (!IAB.isPurchased(ActivityPro.SKU_LOG, this@ActivityMain))
                        prefs.edit().putBoolean("log", false).apply()
                    if (!IAB.isPurchased(ActivityPro.SKU_THEME, this@ActivityMain)) {
                        if ("teal" != prefs.getString("theme", "teal"))
                            prefs.edit().putString("theme", "teal").apply()
                    }
                    if (!IAB.isPurchased(ActivityPro.SKU_NOTIFY, this@ActivityMain))
                        prefs.edit().putBoolean("install", false).apply()
                    if (!IAB.isPurchased(ActivityPro.SKU_SPEED, this@ActivityMain))
        ...
        llSupport.setOnClickListener {
            startActivity(getIntentPro(this@ActivityMain))
        }
    ...
    override fun onResume() {
        ...
        val llSupport = findViewById<LinearLayout>(R.id.llSupport)
        llSupport.visibility = if (IAB.isPurchasedAny(this) || getIntentPro(this).resolveActivity(pm) == null)
            View.GONE else View.VISIBLE

        val canNotify = (Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU ||
                (ContextCompat.checkSelfPermission(this, Manifest.permission.POST_NOTIFICATIONS) == PackageManager.PERMISSION_GRANTED))
        val tvNotifications = findViewById<TextView>(R.id.tvNotifications)
        tvNotifications?.visibility = if (canNotify) View.GONE else View.VISIBLE
    ...
    override fun onSharedPreferenceChanged(prefs: SharedPreferences, name: String?) {
        ...
        if ("enabled" == name) {
        ...
        } else if ("whitelist_wifi" == name ||
            "screen_on" == name ||
            "screen_wifi" == name ||
            "whitelist_other" == name ||
            "screen_other" == name ||
            "whitelist_roaming" == name ||
            "show_user" == name ||
            "show_system" == name ||
            "show_nointernet" == name ||
            "show_disabled" == name ||
            "sort" == name ||
            "imported" == name
        ) {
            ...
            val whitelistWifi = prefs.getBoolean("whitelist_wifi", false)
            val whitelistOther = prefs.getBoolean("whitelist_other", false)
            val hintWhitelist = prefs.getBoolean("hint_whitelist", true)
            llWhitelist.visibility = if (!(whitelistWifi || whitelistOther) && screenOn && hintWhitelist) View.VISIBLE else View.GONE
        ...
        } else if ("theme" == name || "dark_theme" == name)
            recreate()
    }

    private val accessChangedListener = DatabaseHelper.AccessChangedListener {
        runOnUiThread {
            if (adapter?.isLive == true)
                adapter?.notifyDataSetChanged()
        }
    }
            ...
            if (adapter != null)
                if (intent.hasExtra(EXTRA_CONNECTED) && intent.hasExtra(EXTRA_METERED)) {
                    ivIcon?.setImageResource(
                        if (Util.isNetworkActive(this@ActivityMain))
    ...
    override fun onCreateOptionsMenu(menu: Menu): Boolean {
        ...
        menuSearch?.setOnActionExpandListener(object : MenuItem.OnActionExpandListener {
            ...
            override fun onMenuItemActionCollapse(item: MenuItem): Boolean {
                if (intent.hasExtra(EXTRA_SEARCH) && !intent.getBooleanExtra(EXTRA_RELATED, false))
                    finish()
                return true
            }
        })
        ...
        if (getIntentSupport().resolveActivity(packageManager) == null)
    ...
    private fun markPro(menuItem: MenuItem, sku: String?) {
    ...
    override fun onPrepareOptionsMenu(menu: Menu): Boolean {
        ...
        menu.findItem(R.id.menu_malware).isVisible = false
        ...
        if ("uid" == sort)
            menu.findItem(R.id.menu_sort_uid).isChecked = true
        else
        ...
        menu.findItem(R.id.menu_malware).isChecked = prefs.getBoolean("malware", false)
    ...
    private fun showHints() {
        ...
        val whitelistOther = prefs.getBoolean("whitelist_other", false)
        val hintWhitelist = prefs.getBoolean("hint_whitelist", true)
        llWhitelist.visibility = if (!(whitelistWifi || whitelistOther) && hintWhitelist && !hintUsage) View.VISIBLE else View.GONE
    ...
    private fun updateApplicationList(search: String?) {
        ...
        object : AsyncTask<Any?, Any?, List<Rule>>() {
            ...
            override fun doInBackground(vararg arg: Any?): List<Rule> {
    ...
    private fun checkDataSaving() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            if (Util.dataSaving(this) && packageManager.resolveActivity(settings, 0) != null)
                try {
                    val prefs = PreferenceManager.getDefaultSharedPreferences(this)
    ...
    private fun menu_malware(item: MenuItem) {
        item.isChecked = !item.isChecked
        val prefs = PreferenceManager.getDefaultSharedPreferences(this)
        ...
        if (item.isChecked)
            try {
                ...
                DownloadTask(this, URL(MALWARE_URL), file, object : DownloadTask.Listener {
                    override fun onCompleted() {
                        prefs.edit().putBoolean("filter", true).apply()
                        ServiceSinkhole.reload("malware download", this@ActivityMain, false)
                    }
                    ...
                    override fun onException(ex: Throwable) {
                        Toast.makeText(this@ActivityMain, ex.message, Toast.LENGTH_LONG).show()
                    }
                }).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR)
    ...
    companion object {
        ...
        private const val MALWARE_URL = "https://urlhaus.abuse.ch/downloads/hostfile/"

        private fun getIntentPro(context: Context): Intent {
            return if (Util.isPlayStoreInstall(context))
                Intent(context, ActivityPro::class.java)
            else {
                Intent(Intent.ACTION_VIEW).apply {
                    data = Uri.parse("https://contact.faircode.eu/?product=netguardstandalone")
                }
            }
        }
        ...
        private fun getIntentInvite(context: Context): Intent {
            return Intent(Intent.ACTION_SEND).apply {
                ...
                putExtra(Intent.EXTRA_TEXT, context.getString(R.string.msg_try) + "\n\nhttps://www.netguard.me/\n\n")
            }
        }
        ...
        private fun getIntentRate(context: Context): Intent {
            ...
            if (intent.resolveActivity(context.packageManager) == null)
                intent = Intent(Intent.ACTION_VIEW, Uri.parse("https://play.google.com/store/apps/details?id=" + context.packageName))
            return intent
        }

        private fun getIntentSupport(): Intent {
            return Intent(Intent.ACTION_VIEW).apply {
                data = Uri.parse("https://github.com/M66B/NetGuard/blob/master/FAQ.md")
            }
        }
    }
}
