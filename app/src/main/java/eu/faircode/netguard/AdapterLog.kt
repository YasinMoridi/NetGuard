...
class AdapterLog(context: Context, cursor: Cursor?, private var resolve: Boolean, private var organization: Boolean) :
    CursorAdapter(context, cursor, 0) {

    private val colTime: Int = cursor?.getColumnIndex("time") ?: -1
    private val colVersion: Int = cursor?.getColumnIndex("version") ?: -1
    private val colProtocol: Int = cursor?.getColumnIndex("protocol") ?: -1
    private val colFlags: Int = cursor?.getColumnIndex("flags") ?: -1
    private val colSAddr: Int = cursor?.getColumnIndex("saddr") ?: -1
    private val colSPort: Int = cursor?.getColumnIndex("sport") ?: -1
    private val colDAddr: Int = cursor?.getColumnIndex("daddr") ?: -1
    private val colDPort: Int = cursor?.getColumnIndex("dport") ?: -1
    private val colDName: Int = cursor?.getColumnIndex("dname") ?: -1
    private val colUid: Int = cursor?.getColumnIndex("uid") ?: -1
    private val colData: Int = cursor?.getColumnIndex("data") ?: -1
    private val colAllowed: Int = cursor?.getColumnIndex("allowed") ?: -1
    private val colConnection: Int = cursor?.getColumnIndex("connection") ?: -1
    private val colInteractive: Int = cursor?.getColumnIndex("interactive") ?: -1
    ...
    override fun bindView(view: View, context: Context, cursor: Cursor) {
        ...
        // Show time
        tvTime.text = SimpleDateFormat("HH:mm:ss", Locale.US).format(time)
        ...
        // strip off user ID
        val displayUid = uid % 100000
        if (displayUid == -1) {
            tvUid.text = ""
        } else if (displayUid == 0) {
            tvUid.text = context.getString(R.string.title_root)
        } else if (displayUid == 9999) {
            tvUid.text = "-" // nobody
        } else {
            tvUid.text = displayUid.toString()
        }
